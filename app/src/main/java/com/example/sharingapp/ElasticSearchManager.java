...
public class ElasticSearchManager {
    ...
    private static final String INDEX = "712147012"; // TODO: MUST CHANGE THIS to random number string
    ...
    public static class AddBidTask extends AsyncTask<Bid,Void,Boolean> {
        ...
        protected Boolean doInBackground(Bid... params) {
            ...
            String bidId = bid.getBidId(); // Explicitly set the id to match the locally generated id
            Index index = new Index.Builder(bid).index(INDEX).type(BID_TYPE).id(bidId).build();
            try {
                DocumentResult execute = client.execute(index);
                if(execute.isSucceeded()) {
                    Log.i("ELASTICSEARCH", "Add item was successful");
